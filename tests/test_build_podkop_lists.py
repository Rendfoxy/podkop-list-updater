...
class BuildPodkopListsTests(unittest.TestCase):
    ...
    def test_build_output_excludes_ai_roots_from_russia_source(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            project_root = Path(tmp_dir)
            local_source = project_root / "config" / "manual.txt"
            exclude_source = project_root / "config" / "exclude.txt"
        ...
        self.assertEqual(domains, ["youtube.com"])

    def test_build_output_can_move_domain_from_foreign_to_russia(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            project_root = Path(tmp_dir)
            ...
            russia_source.write_text("iot334.com\n", encoding="utf-8")
            foreign_source.write_text("iot334.com\n", encoding="utf-8")
            foreign_exclude.write_text("iot334.com\n", encoding="utf-8")
